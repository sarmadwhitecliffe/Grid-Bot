...
def mock_notifier() -> MagicMock:
    """Mock Notifier with no-op alert methods."""
    alerter = MagicMock()
    alerter.alert_grid_deployed = AsyncMock()
    alerter.alert_risk_action = AsyncMock()
    alerter.alert_shutdown = AsyncMock()
    alerter.send_heartbeat = AsyncMock()
    alerter.alert_tier_change = AsyncMock()
    return alerter
...
class TestBotBootSequence:
    ...
    async def test_init_components_succeeds(
        ...
        with (
            patch("main.ExchangeClient", return_value=mock_exchange_client),
            ...
            assert bot._notifier is not None
...
class TestCompleteTradingCycle:
    ...
    async def test_ranging_regime_deploys_grid(
        ...
        mock_notifier.alert_grid_deployed = AsyncMock()
        mock_notifier.alert_risk_action = AsyncMock()
        mock_notifier.alert_shutdown = AsyncMock()
        with (
            patch("main.ExchangeClient", return_value=mock_exchange_client),
            ...
            mock_notifier.alert_grid_deployed.assert_called_once()
    ...
    async def test_trending_regime_cancels_grid(
        ...
        with (
            ...
            patch("main.Notifier", return_value=mock_alerter),
            patch("main.settings", integration_settings),
            patch("config.settings.settings", integration_settings),
        ):
            await bot._init_components()
...
class TestStatePersistence:
    ...
    async def test_state_save_and_load_roundtrip(
        ...
        bot1._order_manager.export_state = MagicMock(
            return_value={
                "order_001": {
                    "order_id": "order_001",
                    "grid_price": 29_850.0,
...
class TestEndToEndSmoke:
    ...
    async def test_full_bot_lifecycle(
        ...
        with (
            ...
            mock_exchange_client.get_order_status = AsyncMock(
                return_value={
                    "id": filled_order_id,
                    "status": "closed",
                    "filled": 0.001,
            ...
            assert bot._running is False
            mock_exchange_client.close.assert_called_once()
            mock_notifier.alert_shutdown.assert_called_once()

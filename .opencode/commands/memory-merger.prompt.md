---
description: "Merges mature lessons from domain memory file into instruction file"
agent: "@memory-keeper"
tools: ["read", "write", "edit", "glob"]
applies-to: "Memory management and knowledge consolidation"
---
...
```
/memory-merger ${input:DomainName} [${input:Scope}]
```

- `${input:DomainName}` - Required. The domain to merge (e.g., `clojure`, `git-workflow`, `prompt-engineering`)
- `${input:Scope}` - Optional. One of: `global`, `user` (both mean global), `workspace`, or `ws`. Defaults to `global`

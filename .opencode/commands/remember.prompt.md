...
applies-to: "Memory management and knowledge persistence"
---
...
```
/remember [${input:DomainName} [${input:Scope}]] ${input:LessonContent}
```

- `${input:DomainName}` - Optional. Explicitly target a domain (e.g., `clojure`, `git-workflow`)
- `${input:Scope}` - Optional. One of: `global`, `user` (both mean global), `workspace`, or `ws`. Defaults to `global`
- `${input:LessonContent}` - Required. The lesson to remember

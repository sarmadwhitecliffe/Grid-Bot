---
description: "Invoke Async Sheriff to validate async/await patterns, rate limiting, and retry logic"
agent: "@async-sheriff"
tools: ["read", "grep", "glob", "bash"]
applies-to: "Grid Bot async module validation and debugging"
---

# Async Pattern Validation for Grid Bot

Validate async/await patterns in Grid Bot modules for compliance and debugging.

## Validation Type

**Validation Type**: `${input:ValidationType|Single Module,Full Phase Audit,Exchange Client Critical,Debug Async Issue}`

## Single Module Validation

For individual module validation, provide:
- **Module Name**: `${input:ModuleName}`
- **File Path**: `${input:FilePath}`

## Phase Validation

For complete phase validation, provide:
- **Phase Number**: `${input:PhaseNumber}`
- **Modules List**: `${input:ModulesList}`

## Debug Async Issues

For debugging async problems, provide:
...
All validations check for:
- ✅ `import ccxt.async_support as ccxt`
- ✅ `enableRateLimit=True` on all exchange instances  
- ✅ All async exchange methods properly `await`-ed
- ✅ Exponential backoff retry [1, 2, 5] seconds, max 3 attempts
- ✅ Exception handling catches only `NetworkError` and `RequestTimeout`
- ✅ No blocking I/O operations
- ✅ Proper async context managers
- ✅ Timeouts on all network calls

**Remember:** Async bugs are hard to debug. Validate patterns early and often to prevent production incidents.

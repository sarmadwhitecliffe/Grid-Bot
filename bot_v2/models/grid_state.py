...
class GridState:
    ...
    # Shutdown tracking (for smart resume on reboot)
    shutdown_time: Optional[datetime] = None
    shutdown_reason: Optional[str] = None
    should_resume: bool = True  # False for safety stops like PORTFOLIO_HALT, EMERGENCY
    deployment_count: int = 0  # Track number of deployments for this session
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "centre_price": decimal_to_str(self.centre_price)
    ...
    def from_dict(cls, data: dict) -> "GridState":
        ...
        return cls(
            ...
            centre_price=to_decimal(data.get("centre_price"))
            if data.get("centre_price")
            else None,
            ...
            initial_equity=to_decimal(data.get("initial_equity", "0")) or Decimal("0"),
            current_equity=to_decimal(data.get("current_equity", "0")) or Decimal("0"),
            total_fees=to_decimal(data.get("total_fees", "0")) or Decimal("0"),
            grid_fills=data.get("grid_fills", 0),
            counter_fills=data.get("counter_fills", 0),
            last_tick_time=datetime.fromisoformat(data["last_tick_time"])
            if data.get("last_tick_time")
            else None,
            shutdown_time=datetime.fromisoformat(data["shutdown_time"])
            if data.get("shutdown_time")
            else None,
            shutdown_reason=data.get("shutdown_reason"),
            should_resume=data.get("should_resume", True),
